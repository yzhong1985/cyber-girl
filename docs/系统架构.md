...
flowchart TB
    ...
    LLM -- "生成的回复文本" --> LLMC
    LLMC --> LOP --> TTS
    LOP -. "拿到回复文字后, 延迟1.6~2.6s, 按字数挑时长匹配的过渡语" .-> HTTP
...
sequenceDiagram
    ...
    participant P as LMOutputProcessor
    ...
    V->>S: 说完的这段音频
    S->>L: 识别出的文本
    L-->>S: 生成的回复文本(有prompt前缀缓存, 稳态约0.2~1s)
    S->>P: 回复文本
    P->>T: 转发给TTS(不阻塞)
    par 铺垫语(占位, 近乎零成本, 按回复字数挑时长匹配的过渡语)
        P->>P: 延迟 1.6~2.6s(模拟反应停顿)
        P->>D: GET /play_filler?chars=N
...
- **铺垫语是"近乎零成本"的旁路，不是真流式**：过渡语视频是启动时预生成、缓存好的，`/play_filler` 只是从池子里挑一个推给播放队列；真正的回复生成(STT→LLM→TTS→Ditto)在各自线程里正常跑，两者天然并行，不需要额外的并发设计。触发点特意放在 LMOutputProcessor(拿到 LLM 回复文字之后)而不是 VAD(说完话那一刻)，这样才知道回复有多长，能挑一段时长占比相近的过渡语，而不是纯随机瞎配。
